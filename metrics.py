...
def standardize_caption(old_json):
    old_cap = json.load(open(old_json,'r'))
    new_cap = []
    for i,(ids) in enumerate(old_cap['imgid']):
        new_cap.append({'image_id':int(ids), 'caption':old_cap['annotations'][i]})
