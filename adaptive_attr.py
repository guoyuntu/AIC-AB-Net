...
class AttentiveCNN( nn.Module ):
    def __init__( self, embed_size, hidden_size ):
        ...
        # ResNet-152 backend
        resnet = models.resnet152( pretrained=True ) # 10 layers in total, freeze the first 6 layers
        child_counter = 0
...
class Decoder( nn.Module ):
    def __init__( self, embed_size, vocab_size, hidden_size ):
        ...
        self.attr_embed = nn.Embedding( vocab_size, int(embed_size / 5) )
        
        # LSTM decoder: input = [ w_t; v_g; attr ] => 2 x word_embed_size;
        self.LSTM = nn.LSTM( embed_size * 3, hidden_size, 1, batch_first=True )
    ...
    def forward( self, V, v_g , attr, captions, states=None ):
        
        # Word Embedding
        embeddings = self.embed( captions )
        attr_emb = self.attr_embed( attr )
        attr_emb = attr_emb.view(attr_emb.size(0), -1)
        
        # x_t = [w_t;attributes;v_g]
        x = torch.cat( ( embeddings, attr_emb.unsqueeze( 1 ).expand_as( embeddings ), v_g.unsqueeze( 1 ).expand_as( embeddings ) ), dim=2 )
        ...
        for time_step in range( x.size( 1 ) ):
            ...
            hiddens[ :, time_step, : ] = h_t.view(h_t.shape[0], h_t.shape[-1])  # Batch_first
...
class Encoder2Decoder( nn.Module ):
    ...
    def forward( self, images, attr, captions, lengths ):
        ...
        scores, _, _,_ = self.decoder( V, v_g, attr, captions )
        
        # Pack it to make criterion calculation more efficient
        packed_scores = pack_padded_sequence( scores, lengths, batch_first=True, enforce_sorted=False )
        
        return packed_scores
    
    # Caption generator
    def sampler( self, images, attr, max_len=30 ):
        """
        ...
        for i in range( max_len ):
            ...
            scores, states, atten_weights, beta = self.decoder( V, v_g, attr, captions, states ) 
